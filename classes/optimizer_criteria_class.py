"""
CourseOptimizerCriteria defines the criteria for the course_level_optimizer to calculate the
 rating value of a single Course object.
"""
...
class CourseOptimizerCriteria(BaseModel):
    ...
    times_start: List of the earliest allowed start time per day.
        Follows datetime.datetime.weekday() index convention, (0 = Monday, 1 = Tuesday, ..., 
         6 = Sunday). Use case: User wants to ensure they start school after a certain time each 
         day.
    times_start_weight:
    times_end: List of the latest allowed end time per day.
        Follows datetime.datetime.weekday() index convention, (0 = Monday, 1 = Tuesday, ..., 
         6 = Sunday). Use case: User wants to ensure they get off from school by a certain time 
         each day.
    times_end_weight:
    ...
    high_prof_rating: Minimum professor rating.
        Should be within range [0, 1], logically a 0 rating would be the same as None. Use case: 
         User wants to get actually decent profs.
    ...
    # Note all the defaults for criteria is None, except for available_times which defaults to an
    #  empty list.
    ...
    def course_eval(self, course: Course) -> float:
        ...
        # Notice available_times and is_virtual ratings depend on the actual meetings count. This
        #  is to help account for variations of meeting counts. For example, a user wants virtual
        #  classes (Optimizer criteria is_virtual=True) as they don't want to commute to campus as
        #  much. Suppose the optimizer is forced to decide between a biweekly in-person lab versus
        #  a weekly in-person lecture. It will rate the biweekly lab to be higher (more favourable)
        #  than the weekly lecture because it has more meetings.
